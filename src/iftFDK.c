...
int iftGenerateTomoFilter(iftComplex* H, int nFilterPts, double dSignalXInc, int bWindow)
{
	...
	{
		(*Hi).r =  -0.5 / ( (double)(i * i) * PI * PI * dSignalXInc * dSignalXInc);
		(*Hf).r = (*Hi).r;
		Hi += 2; Hf -= 2;
	}
	...
	if(mid%2 != 0) //check to see if midpoint is even
		(*(H+mid)).r = -0.5 / ( (double)(mid * mid) * PI * PI * dSignalXInc * dSignalXInc);
...
int iftFDKConeBeam(double *proj,                    //projection data
		   ...
		   )
{
	...
	//increase nSignalXPts and change centerX (0.0 pt) to account for extended projections
	...
	double dBeta = 2.0 * PI / (double)(numProj);
 	...
 	for(iProj = 0; iProj < nProj; iProj++)
	  {
		...
		///////////////////////////////////
		disFromCenterY = ((double)l - centerY ) * dSignalYInc;
		...
		//normalize by nFilterPts and put filtered projection back into original 
		//                    projection image
