...
void FLIMGraphExtractFeaturesFromLayerPerBatch_omp(iftFileSet *fs_graphs, int first, int last, char *orig_dir, iftFLIMArch *arch,
                                                   char *param_dir, int layer_index, char *feat_dir, char *object_dir, int opt)
{
    iftFLIMGraph **input = NULL, **output = NULL;
    iftImage **mask = NULL;
    char filename[200];
...
/* Load batch */
#pragma omp parallel for shared(basename, input, fs_graphs, ext, orig_dir, first, last) private(filename) // OK
    for (int i = first; i <= last; i++)
    {
        sprintf(filename, "%s/%s", orig_dir, fs_graphs->files[i]->path);
        input[i - first] = iftReadFLIMGraph_omp(filename, opt);
        basename[i - first] = iftFilename(fs_graphs->files[i]->path, ext);
    }

    /* For each layer do */
    output = FLIMGraphConvolutionalLayer_omp(input, ngraphs, mask, arch, 0, layer_index, 1, param_dir, opt); // ALREADY TESTED
    // output = FLIMGraphConvolutionalLayer(input, ngraphs, mask, arch, 0, layer_index, 1, param_dir);

#pragma omp parallel for if(opt ==23) shared(input, output, ngraphs) // TESTING
    for (int k = 0; k < ngraphs; k++)
    {
        sprintf(filename, "%s/%s.json", feat_dir, basename[k]);
        iftFree(basename[k]);
        iftWriteFLIMGraph(output[k], filename);
    }

    for (int k = 0; k < ngraphs; k++)
    {
        iftDestroyFLIMGraph(&input[k]);
        iftDestroyFLIMGraph(&output[k]);
    }
...
void FLIMGraphExtractFeaturesFromLayerPerImage_omp(iftFileSet *fs_graphs, int first, int last, char *orig_dir, iftFLIMArch *arch,
                                                   char *param_dir, int layer_index, char *feat_dir, char *object_dir, int opt)
{
    iftFLIMGraph **input = NULL, **output = NULL;
    iftImage **mask = NULL;
    ...
    for (int i = first; i <= last; i++)
    {
        ...
        input[0] = iftReadFLIMGraph_omp(filename, opt);

        char *basename = iftFilename(fs_graphs->files[i]->path, ext);
        if (object_dir != NULL)
        {
            iftWarning("Object mask not supported for graphs", "FLIMGraphExtractFeaturesFromLayerPerImage_omp");
        }

        // printf("Processing file %s: %d of %d files\r", basename, i + 1, last - first + 1);
        // fflush(stdout);

        output = FLIMGraphConvolutionalLayer_omp(input, 1, mask, arch, 0, layer_index, 1, param_dir, opt);
        iftDestroyFLIMGraph(&input[0]);
        input[0] = output[0];
        output[0] = NULL;
        iftFree(output);
...
// dado um conjunto de vértices raíz, insere em neighbors os vizinhos de cada nó em roots.
// a cada inserção, inclui em dissimilarity[q] a distância entre as features do nó referência (feature_reference) e as features do nó q
...
{
    iftSet *neighbors = NULL;
    iftSet *seed = roots;
    while (seed != NULL)
    {
        int node_index = seed->elem;

        iftFLIMGraphNodeList *neigbors_list = graph->nodes[node_index].neighbors_list_head;
        while (neigbors_list != NULL)
        {
            ...
            if (dissimilarity[neigbors_list->node->index] == INFINITY)
            {

                dissimilarity[neigbors_list->node->index] = sqrt(dist);
                iftInsertSet(&neighbors, neigbors_list->node->index);
                (num_neighbors)++;
            }
...
iftDataSet *iftCreateDataSet_omp(int nsamples, int nfeats, int opt)
{
...
#pragma omp parallel for shared(Z, nfeats)
    ...
    Z->data = iftCreateMatrix_omp(Z->nfeats, Z->capacity);

#pragma omp parallel for shared(Z, nsamples)
...
iftDataSet *ComputeSeedDataSetGraph_omp(iftFLIMGraph *graph, iftLabeledSet *S, int *kernel_size, int nsamples, int opt)
{

    int kernel_neighbors = iftMin(graph->num_nodes, kernel_size[0] * kernel_size[1]);
    int tensor_size = graph->num_feats * kernel_neighbors;

    iftDataSet *Z = iftCreateDataSet_omp(nsamples, tensor_size, opt);

    Z->nclasses = 0;
    int id = 0;
    iftLabeledSet *seed = S;

    int *is_seed = (int *)calloc(graph->num_nodes, sizeof(int));
    while (seed != NULL)
    {
        is_seed[seed->elem] = seed->label;
        seed = seed->next;
    }

#pragma omp parallel for shared(Z, nsamples, id, is_seed, graph, kernel_neighbors, tensor_size) schedule(dynamic, 1)
...
#pragma omp critical
            ...
            Z->sample[s].id = s;
            Z->sample[s].truelabel = is_seed[p];
            if (Z->sample[s].truelabel > Z->nclasses)
                Z->nclasses = Z->sample[s].truelabel;
...
iftMatrix *ComputeKernelBank_omp(iftDataSet *Z, int *ngroups, uchar grouping_method, int opt)
{
    iftMatrix *kernels = NULL;
    iftGraph *graph = NULL; /* For IOPF and IW */
    iftKnnGraph *knngraph = NULL;
    ...
    { /* use all markers instead */
        *ngroups = Z->nsamples;
        ...
        // kernels = iftCreateMatrix_omp(*ngroups, Z->nfeats, opt); // NEED TO BE TESTED
        kernels = iftCreateMatrix(*ngroups, Z->nfeats);
        // printf("*ngroups >= Z->nsamples \n");
        // #pragma omp parallel for shared(Z, kernels) // NEED TO BE TESTED
        for (int s = 0; s < Z->nsamples; s++)
        {
            iftUnitNorm(Z->sample[s].feat, Z->nfeats);
            // #pragma omp parallel for shared(Z, kernels, s) // NEED TO BE TESTED
            for (int j = 0; j < Z->nfeats; j++) // Z->nfeats = graph->num_feats * kernel_neighbors
            {
                // kernels contém em cada coluna o kernel de uma semente
                // e seu numero de linhas corresponde ao número de vizinhos*features do grafo
                iftMatrixElem(kernels, s, j) = Z->sample[s].feat[j];
            }
            // i++;
        }
        return (kernels);
    ...
    case 0: /* KMeans */
        ...
        iftKMeans(Z, *ngroups, 100, 0.001, NULL, NULL, iftEuclideanDistance);
        groupsize = iftAllocIntArray(iftCountNumberOfGroupsDataSet(Z) + 1);
        int num_groups = iftCountNumberOfGroupsDataSet(Z) + 1;

// printf("PERFORMING KMEANS \n");
#pragma omp parallel for if (opt ==7) shared(Z) reduction(+ : groupsize[ : num_groups]) // OK
        for (int s = 0; s < Z->nsamples; s++)
        {
            groupsize[Z->sample[s].group]++;
        }

        // #pragma omp parallel for shared(Z, groupsize, mingroupsize, kernels) // NOP
        for (int s = 0; s < Z->nsamples; s++)
        {
            if (iftHasSampleStatus(Z->sample[s], IFT_PROTOTYPE))
            {
                ...
                {
                    iftUnitNorm(Z->sample[s].feat, Z->nfeats);
#pragma omp parallel for if (opt ==8) shared(Z, kernels) // OK
                    for (int j = 0; j < Z->nfeats; j++)
                    {
                        iftMatrixElem(kernels, i, j) = Z->sample[s].feat[j];
                    }
...
void NormalizeGraphByZScore_omp(iftFLIMGraph *graph, float *mean, float *stdev, int opt)
{
    int num_feats = graph->num_feats;

    if(opt == 5){
        #pragma omp parallel for shared(graph, num_feats, mean, stdev)
    for (int p = 0; p < graph->num_nodes; p++)
    {
        for (int b = 0; b < num_feats; b++)
        {
        ...
        #pragma omp parallel for if(opt == 26 || opt == -1) shared(graph, num_feats, mean, stdev) collapse(2)
    for (int p = 0; p < graph->num_nodes; p++)
    {
        for (int b = 0; b < num_feats; b++)
        {
...
iftMatrix *SelectRelevantKernelsByKmeans_omp(iftMatrix *K, int ngroups, int opt)
{
    iftMatrix *Kt = iftTransposeMatrix(K);
    iftDataSet *Z = iftFeatureMatrixToDataSet(Kt);
    iftDestroyMatrix(&Kt);
    ...
    for (s = 0; s < Z->nsamples; s++)
    {
        ...
        {
            iftUnitNorm(Z->sample[s].feat, Z->nfeats);
#pragma omp parallel for if(opt == 12 || opt == -1) shared(kernels, Z, s) private(j) firstprivate(i)
...
iftMatrix *LearnKernelBankGraph_omp(iftFLIMGraph *graph, iftLabeledSet *M, int *kernel_size, int nsamples, int nkernels_per_image, int nkernels_per_marker, int opt)
{
    int tensor_size = graph->num_feats * kernel_size[0] * kernel_size[1];
    ...
    // para cada rótulo c, ou seja, para cada marcador
    // calcula seus kernels (uma matriz), armazenando-os em kernels[c]
    // #pragma omp parallel for shared(kernels, Z, nkernels_per_marker) reduction(+:total_nkernels) // NOP
    for (int c = 1; c <= Z->nclasses; c++)
    {
        iftDataSet *Z1 = iftExtractSamplesFromClass_omp(Z, c, opt); // OK

        int nkernels = nkernels_per_marker;
        /* 0: kmeans, 1: iterated watershed and 2: OPF c clusters */
        kernels[c] = ComputeKernelBank_omp(Z1, &nkernels, 0, opt); // (MISSING TESTING *ngroups >= Z->nsamples)
    ...
    // Seu tamanho é [nkernels][graph->num_feats * kernel_size[0] * kernel_size[1]],
    // em que nkernels é
    // iftMatrix *kernelbank = iftCreateMatrix_omp(total_nkernels, tensor_size, opt);
    iftMatrix *kernelbank = iftCreateMatrix(total_nkernels, tensor_size);
...
#pragma omp parallel for if(opt == 9) shared(kernels, kernelbank, k, c) // OK
            for (int col = 0; col < kernels[c]->ncols; col++)
            {
#pragma omp parallel for if(opt == 10) shared(kernelbank, kernels, c, col, k) // OK
                for (int row = 0; row < kernels[c]->nrows; row++)
                {
                    iftMatrixElem(kernelbank, k + col, row) = iftMatrixElem(kernels[c], col, row);
                }
            }
            k += kernels[c]->ncols;
        }
...
#pragma omp parallel for if(opt == 11 || opt == -1) shared(kernels, kernelbank, k, c) collapse(2) //
            for (int col = 0; col < kernels[c]->ncols; col++)
            {
                for (int row = 0; row < kernels[c]->nrows; row++)
                {
    ...
    if (kernelbank->ncols > nkernels_per_image)
    {                                                                                                 /* force a number of kernels per image */
        iftMatrix *Rkernels = SelectRelevantKernelsByKmeans_omp(kernelbank, nkernels_per_image, opt); // OK
        iftDestroyMatrix(&kernelbank);
        kernelbank = Rkernels;
    }
...
iftMatrix *iftGraphToFeatureMatrix_omp(iftFLIMGraph *graph, int num_adj, float *fill_band_with, int opt)
{
    // cria a matriz de saída
    // iftMatrix *matrix = iftCreateMatrix_omp(num_adj*graph->num_feats, graph->num_nodes, opt); // NEED TO BE TESTED
    iftMatrix *matrix = iftCreateMatrix(num_adj * graph->num_feats, graph->num_nodes);

#pragma omp parallel for if(opt == 19 || opt == -1) shared(matrix, graph, num_adj, fill_band_with) schedule(dynamic, 1) // OK
    for (int p = 0; p < graph->num_nodes; p++)
    {
        double *dissimilarity = (double *)malloc(graph->num_nodes * sizeof(double));
...
/* Create from the graph's nodes a matrix with size [num nodes][num_adj * num features],
   ...
   The similarity measure is euclidean distance in feature (e.g, color) space.
*/
iftMatrix *iftGraphToFeatureMatrix_bkp(iftFLIMGraph *graph, int num_adj, float *fill_band_with)
{
    // cria a matriz de saída
    iftMatrix *matrix = iftCreateMatrix(num_adj * graph->num_feats, graph->num_nodes);
    ...
    for (int p = 0; p < graph->num_nodes; p++)
    {
        ...
        dissimilarity[p] = 0.0;
        iftInsertDHeap(maxHeap, p);
        iftRemoveDHeap(maxHeap);
        iftInsertDHeap(minHeap, p);
        iftInsertSet(&roots, p); // initial nodes

        int found_neighbors = 0;

        while (found_neighbors < num_adj)
        {
            int k = num_adj - found_neighbors;
            getNeighborsFLIMGraph(graph, roots, k, dissimilarity, graph->nodes[p].feats, maxHeap);

            iftDestroySet(&roots);
            if (iftEmptyDHeap(maxHeap))
                break; // there are no neighbors to include

            bool new_iter = (maxHeap->last + 1 < k);
        ...
        while (!iftEmptyDHeap(minHeap))
        {
            ...
            for (int b = 0; b < graph->num_feats; b++)
            {
                iftMatrixElem(matrix, b + i * graph->num_feats, p) = graph->nodes[node_index].feats[b];
            }
            i++;
        }
        iftResetDHeap(minHeap);
        ...
        while (i < num_adj)
        {
            for (int b = 0; b < graph->num_feats; b++)
            {
                iftMatrixElem(matrix, b + i * graph->num_feats, p) = tmp[b];
            }
            i++;
        }
    }
...
iftFLIMGraph **
FLIMGraphConvolutionalLayer_omp(iftFLIMGraph **graph, int ngraphs, iftImage **mask, iftFLIMArch *arch, int layer, int layer_index, int atrous_factor, char *param_dir, int opt)
{
    // Set input parameters
    ...
    // Apply convolutional layer
    // #pragma omp parallel for shared(ngraphs, graph, output_graph, kernelbank, mean, stdev, bias, use_bias, arch, layer, atrous_factor) if(omp_get_max_threads() > ngraphs) // OK
    for (int i = 0; i < ngraphs; i++)
    {
        // marker-based normalization
        ...
        if (use_bias == NULL)
        {
            NormalizeGraphByZScore_omp(graph[i], mean, stdev, opt);
            imgM = iftGraphToFeatureMatrix_omp(graph[i], arch->layer[layer].kernel_size[0] * arch->layer[layer].kernel_size[1], NULL, opt); // OK
            // imgM = iftGraphToFeatureMatrix(graph[i], arch->layer[layer].kernel_size[0]*arch->layer[layer].kernel_size[1], NULL);
        }
        else
        {
            imgM = iftGraphToFeatureMatrix_omp(graph[i], arch->layer[layer].kernel_size[0] * arch->layer[layer].kernel_size[1], mean, opt); // OK
            // imgM = iftGraphToFeatureMatrix(graph[i], arch->layer[layer].kernel_size[0]*arch->layer[layer].kernel_size[1], mean);
        }
...
#pragma omp parallel for
        ...
        if (arch->layer[layer].relu)
        {                                             // ReLU in place
#pragma omp parallel for if(opt == 20 || opt == -1) shared(conv, use_bias, bias) // OK
            for (int p = 0; p < conv->nrows; p++)
            {
                for (int b = 0; b < conv->ncols; b++)
                {
        ...
        if (strcmp(arch->layer[layer].pool_type, "no_pool") != 0)
        {
            output_graph[i] = iftMatrixToFLIMGraph_omp(conv, graph[i], opt); // OK
            // output_graph[i] = iftMatrixToFLIMGraph(conv, graph[i]);
            ...
            { // ignore the stride to learn the model
                ...
                iftFLIMGraphAtrousAveragePooling_omp(conv, output_graph[i], arch->layer[layer].pool_size[0], arch->layer[layer].pool_size[1], arch->layer[layer].pool_size[2], atrous_factor, arch->layer[layer].pool_stride, opt); // NOT TESTED YET
                // iftFLIMGraphAtrousAveragePooling(conv, output_graph[i], arch->layer[layer].pool_size[0], arch->layer[layer].pool_size[1], arch->layer[layer].pool_size[2], atrous_factor, arch->layer[layer].pool_stride);
            }
            else
            {
                ...
                { // ignore the stride to learn the model
                    ...
                    iftFLIMGraphAtrousMaxPooling_omp(conv, output_graph[i], arch->layer[layer].pool_size[0], arch->layer[layer].pool_size[1], arch->layer[layer].pool_size[2], atrous_factor, arch->layer[layer].pool_stride, opt); // OK
                    // iftFLIMGraphAtrousMaxPooling(conv, output_graph[i], arch->layer[layer].pool_size[0], arch->layer[layer].pool_size[1], arch->layer[layer].pool_size[2], atrous_factor, arch->layer[layer].pool_stride);
                }
                else
                {
...
if(opt == 23 || opt == -1){
#pragma omp parallel for shared(output_graph, conv) collapse(2) // OK
            for (int j = 0; j < output_graph[i]->num_nodes; j++)
            {
                for (int b = 0; b < output_graph[i]->num_feats; b++)
                {
    ...
    if(opt == 24){
#pragma omp parallel for shared(output_graph, conv)  // 
         for (int j = 0; j < output_graph[i]->num_nodes; j++)
            {
                for (int b = 0; b < output_graph[i]->num_feats; b++)
                {
         ...
         for (int j = 0; j < output_graph[i]->num_nodes; j++)
            {
                #pragma omp parallel for if(opt == 25) shared(output_graph, conv, j)  // 
                for (int b = 0; b < output_graph[i]->num_feats; b++)
                {
                    output_graph[i]->nodes[j].feats[b] = (double)(iftMatrixElem(conv, b, j));
                }
...
iftMatrix *ConsensusKernelbankGraph_omp(iftFileSet *fs_seeds, char *inputdata_dir, int noutput_channels, int opt)
{
    int nkernels = fs_seeds->n;
    iftMatrix **kernels = (iftMatrix **)calloc(nkernels, sizeof(iftMatrix *));
    int ncols = 0;
...
// printf("Load kernels from the training images\n");

/* Load kernels from the training images */
#pragma omp parallel for if(opt == 13 || opt == -1) shared(kernels, nkernels, fs_seeds, inputdata_dir) private(basename, filename) reduction(+ : ncols) // OK
    for (int i = 0; i < nkernels; i++)
    {
...
if(opt == 14 || opt == 15){
    // printf("Copy all kernels into a single matrix \n");
    for (int i = 0; i < nkernels; i++)
    {
#pragma omp parallel for if(opt == 14) shared(kernels, Mkernels, k, i) // OK
        for (int col = 0; col < kernels[i]->ncols; col++)
        {
#pragma omp parallel for if(opt == 15) shared(kernels, Mkernels, k, col, i) // OK
            for (int row = 0; row < kernels[i]->nrows; row++)
            {
                iftMatrixElem(Mkernels, k + col, row) = iftMatrixElem(kernels[i], col, row);
            }
        }
        k += kernels[i]->ncols;
    }
        ...
        for (int i = 0; i < nkernels; i++)
    {
#pragma omp parallel for if(opt == 16 || opt == -1) shared(kernels, Mkernels, k, i) collapse(2) // OK
        for (int col = 0; col < kernels[i]->ncols; col++)
        {
            for (int row = 0; row < kernels[i]->nrows; row++)
            {
    ...
    /* Reduce the number of kernels into the desired number of output channels */
    // printf("Reduce the number of kernels into the desired number of output channels \n");
    ...
    else
    {
        ...
        {
            Rkernels = SelectRelevantKernelsByKmeans_omp(Mkernels, noutput_channels, opt); // PREVIOUSLY TESTED OK
        }
    }
...
#pragma omp parallel for if(opt == 21 || opt == -1) shared(graph, matrix, graph_ref) // OK
    for (int node_index = 0; node_index < graph->num_nodes; node_index++)
    {
        graph->nodes[node_index].numNeighbors = 0;
        graph->nodes[node_index].numPixels = graph_ref->nodes[node_index].numPixels;
        ...
        // #pragma omp parallel for shared(graph, matrix, node_index) // NOT TESTED YET
        for (int b = 0; b < matrix->ncols; b++)
        {
            graph->nodes[node_index].feats[b] = iftMatrixElem(matrix, b, node_index);
        }
...
#pragma omp parallel for if (opt == 2 || opt == 4 || opt == -1) shared(graph, num_feats)
    for (unsigned long spx_index = 0; spx_index < graph->num_nodes; spx_index++)
    {
        graph->nodes[spx_index].numNeighbors = 0;
        graph->nodes[spx_index].neighbors_list_head = NULL;
...
#pragma omp parallel for if (opt == 3 || opt == 4) shared(graph, feats, node_index)
        for (unsigned long feat_index = 0; feat_index < graph->num_feats; feat_index++)
        {
            graph->nodes[node_index].feats[feat_index] = feats->val[feat_index];
        }
...
iftImage *iftGraphToImage_omp(iftFLIMGraph *graph, iftImage *labels, int Imax, int band, int opt)
{
    int p, b = band;
...
#pragma omp parallel for shared(graph) private(p) reduction(min : min) reduction(max : max)
    for (p = 0; p < graph->num_nodes; p++)
    {
        if (graph->nodes[p].feats[b] < min)
            min = graph->nodes[p].feats[b];
    ...
    if (max > min)
    {
#pragma omp parallel for shared(graph, img, Imax, labels, max, min) private(p) reduction(min : final_min) reduction(max : final_max)
        for (p = 0; p < img->n; p++)
        {
            img->val[p] = (int)((double)(Imax) * ((graph->nodes[labels->val[p] - 1].feats[b] - min) / (max - min)));
...
void StatisticsFromAllSeedsGraph_omp(iftFileSet *fs_seeds, char *dir, float *mean, float *stdev, float stdev_factor, int num_feats, int opt)
{
    int nseeds = 0;
    ...
    Para cada arquivo (imagem):
        ...
        - itera sobre as sementes, contando seu número e o somatório de suas features (cor)
    */
#pragma omp parallel for reduction(+ : nseeds) reduction(+ : mean[ : num_feats]) shared(fs_seeds, dir) schedule(dynamic, 1)
    for (int i = 0; i < fs_seeds->n; i++)
    {
        iftFLIMGraph *graph;
        char *basename;
        ...
        // le o grafo
        sprintf(filename, "%s/%s.json", dir, basename);
        graph = iftReadFLIMGraph_omp(filename, opt);
        isValidFLIMGraph(graph);
        ...
        while (S != NULL)
        {
            ...
            int p = iftRemoveLabeledSet(&S, &l);
            nseeds += 1;
            // #pragma omp parallel for shared(graph, num_feats, p) reduction(+:mean[:num_feats])
            for (int b = 0; b < num_feats; b++)
            {
                mean[b] += graph->nodes[p].feats[b];
            }
...
// termina de calcular a média
#pragma omp parallel for shared(mean, num_feats, nseeds)
    for (int b = 0; b < num_feats; b++)
    {
        mean[b] = mean[b] / nseeds;
    }
    ...
    Para cada arquivo (imagem):
        ...
        - itera sobre as sementes, contando seu número e o desvio padrão de suas features (cor)
    */
#pragma omp parallel for reduction(+ : stdev[ : num_feats]) shared(fs_seeds, dir, mean, num_feats)
    for (int i = 0; i < fs_seeds->n; i++)
    {
        char filename[200];
...
// termina de calcular o desvio padrão
#pragma omp parallel for shared(stdev, num_feats, nseeds, stdev_factor)
    for (int b = 0; b < num_feats; b++)
    {
        stdev[b] = sqrtf(stdev[b] / nseeds) + stdev_factor;
    }
...
iftImage **iftGetActivations_omp(iftFLIMGraph *graph, iftImage *labels, int opt)
{
    ...
    for (int i = 0; i < (int)(graph->num_feats); i++)
    {
        iftImage *band = iftGraphToImage_omp(graph, labels, 255, i, opt);
...
/*
void iftFLIMgraphWriteLayer(char *filename_graph, int num_neighbors, char *filename_layer){
    iftFLIMGraph *graph = iftReadFLIMGraph(filename_graph);

    iftMImage *img = iftCreateMImage(graph->num_feats, graph->num_nodes, 1, graph->num_feats);
    for (int i = 0; i < graph->num_nodes; i++)
    {
        for (int b = 0; b < graph->num_feats; b++)
        {
            img->val[i][b] = graph->nodes[i].feats[b];
        }
    }
...
/* Generate input layer */
// printf("Generate input layer \n");
//  read activation files and write in the temp directory
#pragma omp parallel for if (opt == 1) shared(fs_seeds, activ_dir, ext) private(basename, filename) // OK
    for (int i = 0; i < nimages; i++)
    {
        basename = iftFilename(fs_seeds->files[i]->path, "-seeds_graph.txt");
        ...
        iftFLIMGraph *graph = iftReadFLIMGraph_omp(filename, opt); // OK
        isValidFLIMGraph(graph);
    ...
    // printf("Computing mean and std of the graph node seeds\n");
    // StatisticsFromAllSeedsGraph_omp(fs_seeds, "tmp", mean_graph, stdev_graph, arch->stdev_factor, ninput_channels); //
    StatisticsFromAllSeedsGraph(fs_seeds, "tmp", mean_graph, stdev_graph, arch->stdev_factor);
    ...
    Para cada arquivo (grafo):
        ...
        - escreve o kernel bank em "tmp/%s-kernels.npy"
    */
    // printf("Processing %d files\r", nimages);
    // #pragma omp parallel for shared(fs_seeds, nimages, mean_graph, stdev_graph, arch) private(basename, filename) // NOP
    for (int i = 0; i < nimages; i++)
    {
        // lê o arquivo de ativação de "tmp/%s.json" como iftFLIMGraph: graph
        basename = iftFilename(fs_seeds->files[i]->path, "-seeds_graph.txt");
        ...
        /* Apply marker-based image normalization */
        NormalizeGraphByZScore_omp(graph, mean_graph, stdev_graph, opt); // OK
        ...
        kernelbank = LearnKernelBankGraph_omp(graph, M, arch->layer[0].kernel_size, nsamples, arch->layer[0].nkernels_per_image, arch->layer[0].nkernels_per_marker, opt); // OK
        // kernelbank = LearnKernelBankGraph(graph, M, arch->layer[0].kernel_size, nsamples, arch->layer[0].nkernels_per_image, arch->layer[0].nkernels_per_marker);
    ...
    // printf("Call ConsensusKernelbankGraph_omp\n");

    iftMatrix *kernelbank = ConsensusKernelbankGraph_omp(fs_seeds, "tmp", arch->layer[0].noutput_channels, opt); // OK
    // iftMatrix *kernelbank = ConsensusKernelbankGraph(fs_seeds, "tmp", arch->layer[0].noutput_channels);
...
// printf("Compute bias \n");
// #pragma omp parallel for shared(kernelbank, ninput_channels, mean_graph, stdev_graph) reduction(-:bias[:kernelbank->ncols]) // OK (NEED TO VALIDATE AGAIN)
#pragma omp parallel for if(opt == 17 || opt == -1) shared(kernelbank, ninput_channels, mean_graph, stdev_graph) reduction(- : bias[ : kernelbank->ncols]) collapse(3) // OK
        for (int col = 0; col < kernelbank->ncols; col++)
        {
            // #pragma omp parallel for shared(kernelbank, ninput_channels, mean_graph, stdev_graph, col) reduction(-:bias[col]) // OK
            for (int row = 0; row < kernelbank->nrows; row += ninput_channels)
            {
                for (int ch = 0; ch < ninput_channels; ch++)
                {
                    float val = iftMatrixElem(kernelbank, col, row + ch) / stdev_graph[ch];
                    iftMatrixElem(kernelbank, col, row + ch) = val;
                    bias[col] -= (mean_graph[ch] * val);
                }
            }
        }
    ...
    // printf("Write kernel bank and statistics\n");
    ...
    // printf("Apply convolutional layer using the consensus kernel bank and the statistics from all markers \n");

#pragma omp parallel for if(opt ==18) shared(fs_seeds, nimages, arch, param_dir, output_dir, layer_index, atrous_factor) private(filename, basename) // OK
    for (int i = 0; i < nimages; i++)
    {
        iftFLIMGraph **output;
        basename = iftFilename(fs_seeds->files[i]->path, "-seeds_graph.txt");
        ...
        output = FLIMGraphConvolutionalLayer_omp(&graph, 1, NULL, arch, 0, layer_index, atrous_factor, param_dir, opt); // OK
        // output = FLIMGraphConvolutionalLayer(&graph, 1, NULL, arch, 0, layer_index, atrous_factor, param_dir);
    ...
    // printf("updating atrous factor with pooling of current layer \n");
    ...
    // printf(" writing atrous factor on file \n");
    //  writing atrous factor on file
    sprintf(filename, "%s/intrinsic_atrous.txt", param_dir);
    FILE *fp = fopen(filename, "w");
    fprintf(fp, "%d", atrous_factor);
...
void iftFLIMGraphExtractFeaturesFromLayer_omp(char *orig_dir, char *graph_list, iftFLIMArch *arch, char *param_dir, int layer_index,
                                              char *feat_dir, char *object_dir, int device, int opt)
{
    iftFileSet *fs_graphs = iftLoadFileSetFromDirOrCSV(graph_list, 1, 1);
    int ngraphs = fs_graphs->n;
    int batchsize = 0;
    ...
    sprintf(filename, "%s/%s", orig_dir, fs_graphs->files[0]->path);
    input = iftReadFLIMGraph_omp(filename, opt);
    ...
    { /* process in batch */
        ...
        for (int batch = 0; batch < nbatches; batch++)
        {
            ...
            // printf("Processing batch %d of %d batches\n", batch + 1, nbatches);
            // fflush(stdout);
            FLIMGraphExtractFeaturesFromLayerPerBatch_omp(fs_graphs, first, last, orig_dir, arch, param_dir, layer_index, feat_dir, object_dir, opt); // NOT TESTED YET
        }
        int first = ngraphs - nremaining_graphs, last = ngraphs - 1;
        if (first < last)
        {
    ...
    { /* process per image */
        int first = 0, last = ngraphs - 1;
        FLIMGraphExtractFeaturesFromLayerPerImage_omp(fs_graphs, first, last, orig_dir, arch, param_dir, layer_index, feat_dir, object_dir, opt); // NOT TESTED YET
    }
...
#pragma omp parallel for if(opt ==22) shared(graph, conv, width, height, depth, atrous_factor, stride) // NOT TESTED YET
    for (int p = 0; p < graph->num_nodes; p++)
    {
        double *dissimilarity = (double *)malloc(graph->num_nodes * sizeof(double));
...
#pragma omp parallel for if(opt ==22) shared(graph, conv, width, height, depth, atrous_factor, stride) // OK
    for (int p = 0; p < graph->num_nodes; p++)
    {
        double *dissimilarity = (double *)malloc(graph->num_nodes * sizeof(double));
...
iftMatrix *iftFLIMSelectKernelsManual(char *kernel_bank_path, char *selected_kernels_path)
{
    ...
    printf("learned kernels: %d %d \n", input_kernels->nrows, input_kernels->ncols);
    printf("selected kernels: %d %d \n", input_kernels->nrows, nKernels);
