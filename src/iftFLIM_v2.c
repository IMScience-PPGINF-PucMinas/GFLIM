...
#pragma omp parallel for shared(input, output, ngraphs) // 
    for (int k = 0; k < ngraphs; k++)
    {
        sprintf(filename, "%s/%s.json", feat_dir, basename[k]);
        iftFree(basename[k]);
...
#pragma omp parallel for
    ...
    /*clock_t final = clock();
    double total_time_spent = (double)(final - init) / CLOCKS_PER_SEC; // in seconds
    fp = fopen("parallel_regions.csv", "a+");
    fprintf(fp, "iftCreateDataSet_omp1, %d, %lf\n", omp_get_max_threads(), total_time_spent);
    fclose(fp);*/
...
#pragma omp parallel for
    ...
    /*final = clock();
    total_time_spent = (double)(final - init) / CLOCKS_PER_SEC; // in seconds
    fp = fopen("parallel_regions.csv", "a+");
    fprintf(fp, "iftCreateDataSet_omp2, %d, %lf\n", omp_get_max_threads(), total_time_spent);
    fclose(fp);*/
...
iftMatrix *ComputeKernelBank_omp(iftDataSet *Z, int *ngroups, uchar grouping_method, int opt)
{
    ...
    { /* use all markers instead */
        *ngroups = Z->nsamples;
        ...
        // kernels = iftCreateMatrix_omp(*ngroups, Z->nfeats, opt); // 
        kernels = iftCreateMatrix(*ngroups, Z->nfeats);
...
#pragma omp parallel for if (omp_get_max_threads() > Z->nfeats) // OK
            for (int j = 0; j < Z->nfeats; j++)                                       // Z->nfeats = graph->num_feats * kernel_neighbors
            {
                // kernels contém em cada coluna o kernel de uma semente
                // e seu numero de linhas corresponde ao número de vizinhos*features do grafo
        ...
        total_time_spent = (double)(final - init) / CLOCKS_PER_SEC; // in seconds
        fp = fopen("parallel_regions.csv", "a+");
        fprintf(fp, "ComputeKernelBank_omp1, %d, %lf\n", omp_get_max_threads(), total_time_spent);
        fclose(fp);*/
...
// printf("PERFORMING KMEANS \n");
        //init = clock();
#pragma omp parallel for reduction(+ : groupsize[ : num_groups]) // OK
        for (int s = 0; s < Z->nsamples; s++)
        {
            groupsize[Z->sample[s].group]++;
        }
        ...
        total_time_spent = (double)(final - init) / CLOCKS_PER_SEC; // in seconds
        fp = fopen("parallel_regions.csv", "a+");
        fprintf(fp, "ComputeKernelBank_omp2, %d, %lf\n", omp_get_max_threads(), total_time_spent);
        fclose(fp);*/
...
//#pragma omp parallel for if (omp_get_max_threads() > Z->nfeats) // OK
        ...
        total_time_spent = (double)(final - init) / CLOCKS_PER_SEC; // in seconds
        fp = fopen("parallel_regions.csv", "a+");
        fprintf(fp, "ComputeKernelBank_omp3, %d, %lf\n", omp_get_max_threads(), total_time_spent);
        fclose(fp);*/
...
#pragma omp parallel for collapse(2) 
    ...
    /*clock_t final = clock();
    double total_time_spent = (double)(final - init) / CLOCKS_PER_SEC; // in seconds
    FILE *fp = fopen("parallel_regions.csv", "a+");
    fprintf(fp, "NormalizeGraphByZScore_omp, %d, %lf\n", omp_get_max_threads(), total_time_spent);
    fclose(fp);*/
}
...
iftMatrix *SelectRelevantKernelsByKmeans_omp(iftMatrix *K, int ngroups, int opt)
{
    ...
    for (s = 0; s < Z->nsamples; s++)
    {
        ...
        {
            iftUnitNorm(Z->sample[s].feat, Z->nfeats);
#pragma omp parallel for if (omp_get_max_threads() > Z->nfeats) shared(kernels, Z, s) private(j) firstprivate(i)
    ...
    double total_time_spent = (double)(final - init) / CLOCKS_PER_SEC; // in seconds
    FILE *fp = fopen("parallel_regions.csv", "a+");
    fprintf(fp, "SelectRelevantKernelsByKmeans_omp, %d, %lf\n", omp_get_max_threads(), total_time_spent);
    fclose(fp);*/
...
#pragma omp parallel for //collapse(2) //
    ...
    double total_time_spent = (double)(final - init) / CLOCKS_PER_SEC; // in seconds
    FILE *fp = fopen("parallel_regions.csv", "a+");
    fprintf(fp, "LearnKernelBankGraph_omp, %d, %lf\n", omp_get_max_threads(), total_time_spent);
    fclose(fp);*/
...
#pragma omp parallel for shared(matrix, graph, num_adj, fill_band_with) schedule(dynamic, 1) // OK
    for (int p = 0; p < graph->num_nodes; p++)
    {
        double *dissimilarity = (double *)malloc(graph->num_nodes * sizeof(double));
...
#pragma omp parallel for // OK
            ...
            total_time_spent = (double)(final - init) / CLOCKS_PER_SEC; // in seconds
            fp = fopen("parallel_regions.csv", "a+");
            fprintf(fp, "FLIMGraphConvolutionalLayer_omp1, %d, %lf\n", omp_get_max_threads(), total_time_spent);
            fclose(fp);*/
        }
        // pooling
...
#pragma omp parallel for shared(output_graph, conv) //collapse(2) // OK
            ...
            total_time_spent = (double)(final - init) / CLOCKS_PER_SEC; // in seconds
            fp = fopen("parallel_regions.csv", "a+");
            fprintf(fp, "FLIMGraphConvolutionalLayer_omp2, %d, %lf\n", omp_get_max_threads(), total_time_spent);
            fclose(fp);*/
        }
        else
        {
...
/* Load kernels from the training images */
    //init = clock();
    // incluído o schedule(dynamic,1)
#pragma omp parallel for shared(kernels, nkernels, fs_seeds, inputdata_dir) private(basename, filename) reduction(+ : ncols) schedule(dynamic,1) // OK
    for (int i = 0; i < nkernels; i++)
    {
    ...
    total_time_spent = (double)(final - init) / CLOCKS_PER_SEC; // in seconds
    fp = fopen("parallel_regions.csv", "a+");
    fprintf(fp, "ConsensusKernelbankGraph_omp1, %d, %lf\n", omp_get_max_threads(), total_time_spent);
    fclose(fp);*/
    ...
    for (int i = 0; i < nkernels; i++)
    {
#pragma omp parallel for shared(kernels, Mkernels, k, i) collapse(2) // OK
    ...
    total_time_spent = (double)(final - init) / CLOCKS_PER_SEC; // in seconds
    fp = fopen("parallel_regions.csv", "a+");
    fprintf(fp, "ConsensusKernelbankGraph_omp2, %d, %lf\n", omp_get_max_threads(), total_time_spent);
    fclose(fp);*/
...
iftFLIMGraph *iftMatrixToFLIMGraph_omp(iftMatrix *matrix, iftFLIMGraph *graph_ref, int opt)
{
    ...
    //clock_t init = clock();
    // trocou schedule(dynamic) por schedule(dynamic,1)
#pragma omp parallel for schedule(dynamic,1) // OK
    for (int node_index = 0; node_index < graph->num_nodes; node_index++)
    {
        graph->nodes[node_index].numNeighbors = 0;
        graph->nodes[node_index].numPixels = graph_ref->nodes[node_index].numPixels;
        ...
        // #pragma omp parallel for shared(graph, matrix, node_index) // 
        for (int b = 0; b < matrix->ncols; b++)
        {
            graph->nodes[node_index].feats[b] = iftMatrixElem(matrix, b, node_index);
        }
    ...
    double total_time_spent = (double)(final - init) / CLOCKS_PER_SEC; // in seconds
    FILE *fp = fopen("parallel_regions.csv", "a+");
    fprintf(fp, "iftMatrixToFLIMGraph_omp, %d, %lf\n", omp_get_max_threads(), total_time_spent);
    fclose(fp);*/
...
#pragma omp parallel for
    ...
    double total_time_spent = (double)(final - init) / CLOCKS_PER_SEC; // in seconds
    fp = fopen("parallel_regions.csv", "a+");
    fprintf(fp, "iftReadFLIMGraph_omp1, %d, %lf\n", omp_get_max_threads(), total_time_spent);
    fclose(fp);*/
...
//#pragma omp parallel for if (graph->num_feats > omp_get_max_threads()) // removido
        for (unsigned long feat_index = 0; feat_index < graph->num_feats; feat_index++)
        {
            graph->nodes[node_index].feats[feat_index] = feats->val[feat_index];
        }
...
//  read activation files and write in the temp directory
    ...
    //init =  = clock()
//#pragma omp parallel for shared(fs_seeds, activ_dir, ext) private(basename, filename) schedule(dynamic, 1) // OK
    for (int i = 0; i < nimages; i++)
    {
        basename = iftFilename(fs_seeds->files[i]->path, "-seeds_graph.txt");
        ...
        //iftFLIMGraph *graph = iftReadFLIMGraph(filename); // removeu o omp, mas incluiu o iftReadFLIMGraph_omp
        isValidFLIMGraph(graph);
    ...
    total_time_spent = (double)(final - init) / CLOCKS_PER_SEC; // in seconds
    fp = fopen("parallel_regions.csv", "a+");
    fprintf(fp, "iftFLIMGraphLearnLayer_omp1, %d, %lf\n", omp_get_max_threads(), total_time_spent);
    fclose(fp);*/
...
// #pragma omp parallel for shared(kernelbank, ninput_channels, mean_graph, stdev_graph) reduction(-:bias[:kernelbank->ncols]) // OK (NEED TO VALIDATE AGAIN)
    //init = clock();
#pragma omp parallel for shared(kernelbank, ninput_channels, mean_graph, stdev_graph) reduction(- : bias[ : kernelbank->ncols]) collapse(3) // OK
        for (int col = 0; col < kernelbank->ncols; col++)
        {
        ...
        total_time_spent = (double)(final - init) / CLOCKS_PER_SEC; // in seconds
        fp = fopen("parallel_regions.csv", "a+");
        fprintf(fp, "iftFLIMGraphLearnLayer_omp2, %d, %lf\n", omp_get_max_threads(), total_time_spent);
        fclose(fp);*/
...
#pragma omp parallel for shared(graph, conv, width, height, depth, atrous_factor, stride) schedule(dynamic, 1) // NOT TESTED YET
    for (int p = 0; p < graph->num_nodes; p++)
    {
        double *dissimilarity = (double *)malloc(graph->num_nodes * sizeof(double));
...
#pragma omp parallel for shared(graph, conv, width, height, depth, atrous_factor, stride) schedule(dynamic, 1) // OK
    for (int p = 0; p < graph->num_nodes; p++)
    {
        double *dissimilarity = (double *)malloc(graph->num_nodes * sizeof(double));
